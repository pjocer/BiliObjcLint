...
### 修复
- **wrapper_empty_pointer 多行容器误报**：修复多行 `@[]` / `@{}` 在 URL、消息发送、控制流、`return`/赋值语句等场景下的串台误报；对当前文件内明确返回新对象的 builder 方法不再误判为空指针风险
- **block_retain_cycle 误报收敛**：普通类方法 callback block 不再仅因使用 `self` 就提示循环引用；`weakSelf` / `self_weak_` 仅在真正被解引用时才提示 `strongify`

## v1.7.2 (2026-03-18)

### 改进
- **Claude 首屏弹窗收敛**：初始弹窗移除 `自动修复` 和 `忽略全部` 按钮，仅保留 `取消` 与 `查看详情`；所有忽略和自动修复操作统一收敛到 HTML 详情页中执行，降低首屏误触风险

## v1.7.1 (2026-03-18)

### 修复
- **Claude 弹窗按钮超限**：修复初始弹窗传入 4 个按钮导致 `osascript display dialog` 报 `最多允许使用三个按钮 (-50)`、窗口完全不显示的问题；改为 3 按钮布局，取消操作走关闭窗口或 `Esc`
- **Claude 弹窗取消状态**：修复用户关闭弹窗后被错误记为 `Unknown dialog result: None` 的问题，`(-128)` 现在会按取消处理
- **AppleScript 弹窗健壮性**：补充标题与消息转义、显式 `activate`，并将真正的 AppleScript 执行失败提升为明确错误日志，避免静默吞掉

## v1.7.0 (2026-03-16)

### 修复
- **趋势图历史数据库兼容**：修复已部署 `biliobjclint-server` 使用旧数据库时趋势图为空的问题；当历史库缺少 `violation_observations` 明细时，Dashboard 会自动回退到按时间槽聚合历史 run 统计数据进行展示

### 改进
- **趋势图兼容提示**：当趋势图使用历史 run 兼容数据时，Dashboard 会显示说明文案，区分精确明细统计与历史兼容口径

## v1.6.9 (2026-03-16)

### 新增
- **Autofix 行为明细**：Dashboard 新增行为明细表，展示弹窗与详情页中的用户操作统计
- **弹窗“忽略全部”**：Claude 自动修复弹窗新增“忽略全部”选项，可直接批量忽略并结束流程

### 修复
- **趋势图统计口径**：改为按天/按小时精确统计该时间槽内出现过的 `violation_id` 去重数量
- **每日统计口径**：改为汇总当日所有 run 的 `total`、`warning`、`error`，不再显示当前去重快照

### 改进
- **Autofix 时间筛选**：Dashboard 中 Autofix 汇总与行为明细改为按用户点击行为发生时间统计，并兼容旧数据回退显示

## v1.6.8 (2026-03-05)

### 修复
- **Metrics 上报阻塞编译**：修复 metrics endpoint 不可达时，spool 积压重试导致 Lint 耗时从 <1s 膨胀到 60s+ 的问题

### 改进
- **Metrics 上报改为独立后台进程**：Lint 主进程仅写 spool 文件（零网络 I/O），由 `code_style_check.sh` 在 Lint 完成后启动 `upload.py` 后台进程异步上报，完全不阻塞 Xcode 编译
- **新增 TCP 探活机制**：上报前先探测 endpoint 可达性（1 秒超时），不可达时跳过网络请求，直接保留 spool 待下次重发

## v1.6.7 (2026-03-04)

### 新增
- **Violations 页面日期过滤**：新增「开始日期」和「结束日期」筛选控件，支持按 `first_seen` 过滤违规记录
- **今日新增 Violation Type 可点击**：Dashboard 中「今日新增 Violation Type」每行可点击跳转至 Violations 页面，自动携带 `rule_id`、`sub_type` 和当天日期筛选条件

### 修复
- **趋势图数字累加**：修复同一天多次编译导致趋势图数字累加的问题，改为基于 violations 表去重统计
- **Dashboard tooltip 延迟**：趋势图节点 tooltip 改为即时显示，去除浏览器默认延迟

## v1.6.6 (2026-03-04)

### 修复
- **excluded 配置对本地 Pod 无效**：修复 `excluded` 模式（如 `*.framework/**/*`）未应用于本地 Pod 文件的问题，本地 Pod 中的 framework 头文件不再产生误报
...
- **biliobjclint-xcode.sh 未识别 --project-root**：CLI 参数解析补充 `--project-root` 选项
